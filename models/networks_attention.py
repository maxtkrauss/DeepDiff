...
#            self.model = nn.Sequential(*model)
        if not outermost:
            self.attention = Spectral_Atten(dim=outer_nc, heads=4)
        else:
            self.attention = None
...
#            return torch.cat([x, self.model(x)], 1)
    ...
    def forward(self, x):
        if self.outermost:
            ...
            y = self.model(x)
            #print("y.shape before attention:", y.shape)
            if self.attention is not None:
                y = self.attention(y)
            #print("y.shape after attention:", y.shape)
